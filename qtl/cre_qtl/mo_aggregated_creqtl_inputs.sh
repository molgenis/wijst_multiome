#!/usr/bin/env bash
###################################################################
#Script Name	  : mo_aggregated_creqtl_inputs.sh
#Description	  : aggregate all the CRE QTL inputs
#Args           : cre location, folder containing per-sample p/beta/se, folder to output aggregated matrices
#Author       	: Roy Oelen
#example        : /groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/cre_eqtl/pseudobulk_replication/scripts/mo_aggregated_creqtl_inputs.sh \
...
tmp_ps=${OUTPUT_LOC}'/'${uuid}'.betas.tsv'
tmp_betas=${OUTPUT_LOC}'/'${uuid}'.betas.tsv'
tmp_ses=${OUTPUT_LOC}'/'${uuid}'.betas.tsv'
# and full files
