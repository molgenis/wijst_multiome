...
match_to_lcl_replication <- function(caqtl_output_loc, lcl_output_loc, cts=c('B', 'CD4T', 'CD8T', 'DC', 'monocyte', 'NK')) {
  # we'll store matched info per cell type
  lcl_per_ct <- list()
  # check each cell type
  for (ct in cts) {
    # location of the combined B caQTLs
    b_combined_caqtl_significant_loc <- paste(caqtl_output_loc, ct, '/qtl_results_all_qval_allchroms_fdr005_significant.txt.gz', sep = '/')
    ...
    colnames(b_combined_caqtl_significant_top) <- c('qtl', 'mo_zscore', 'mo_assessed_allele', 'mo_significant', 'mo_beta', 'mo_se', 'mo_p')
    ...
    for (chromosome in 1:22) {
      ...
      lcl_chromosome_loc <- paste0(lcl_output_loc, '/qtl_results_all_qval_', chromosome, '.txt.gz')
      ...
      # and rename columns so they don't conflict with the lcl data
      colnames(lcl_chromosome) <- c('qtl', 'lcl_zscore', 'lcl_assessed_allele', 'lcl_significant', 'lcl_beta', 'lcl_se', 'lcl_p')
    ...
    b_combined_top_and_lcl_caqtl[b_combined_top_and_lcl_caqtl[['mo_assessed_allele']] != b_combined_top_and_lcl_caqtl[['lcl_assessed_allele']], 'lcl_zscore_signed'] <- -1 * b_combined_top_and_lcl_caqtl[b_combined_top_and_lcl_caqtl[['mo_assessed_allele']] != b_combined_top_and_lcl_caqtl[['lcl_assessed_allele']], 'lcl_zscore']
    # put in the list
    lcl_per_ct[[ct]] <- b_combined_top_and_lcl_caqtl
  }
  return(lcl_per_ct)
}
...
lcl_caqtl_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/caqtl/replication//lcl/'
...
# get overlap per ct
combined_caqtl_to_lcl <- match_to_lcl_replication(caqtl_output_combined_loc, lcl_caqtl_loc)
...
saveRDS(combined_caqtl_to_lcl, '~/rds/caqtl_all_combined_lcl_replication.rds')
...
# save in a new list
combined_caqtl_to_lcl_reformatted <- list()
# check each cell type
for (ct in names(combined_caqtl_to_lcl)) {
  # extract the table
  ct_table <- combined_caqtl_to_lcl[[ct]]
  # replace 'mo_' with the cell type
  colnames(ct_table) <- gsub('^mo_', paste0(ct, '_'), colnames(ct_table))
  # fix this stupid typo
  # colnames(ct_table) <- gsub('assesed', 'assessed', colnames(ct_table))
  # put it back into the list as LCL vs ct
  combined_caqtl_to_lcl_reformatted[[paste('lcl', 'vs', ct, sep = '_')]] <- ct_table
}
...
lcl_celltype_stats <- get_stats_qtl_matches(combined_caqtl_to_lcl_reformatted, significance_column_replicating_emperical = 'significant', significance_cutoff_replicating_in_emperical = 'significant')
...
# save the image
ggsave('~/plots/mo_caqtl_lcl_replication_alt.pdf', plot = caqtl_lcl_celltype_stat_p_alt, width = 3, height = 8)
...
# and save those
ggsave('~/plots/mo_caqtl_lcl_replication_alt_legend.pdf', plot = caqtl_lcl_celltype_stat_p_alt_legend, width = 1, height = 2)

```

```{r plot_lcl_combined_overlap, include=TRUE, fig.width=8, fig.height=8}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# extract B against LCL
b_combined_top_and_lcl_caqtl <- combined_caqtl_to_lcl_reformatted[['lcl_vs_B']]

# subset the mo to what is significant in our data
b_combined_top_and_lcl_caqtl_mosig <- b_combined_top_and_lcl_caqtl[b_combined_top_and_lcl_caqtl[['B_significant']] == T, ]
...
b_combined_top_and_lcl_caqtl_mosig <- b_combined_top_and_lcl_caqtl_mosig[
  ...
  b_combined_top_and_lcl_caqtl_mosig[['B_significant']] == T, ]
...
min_sig_z_mo <- min(abs(b_combined_top_and_lcl_caqtl_mosig[b_combined_top_and_lcl_caqtl_mosig[['B_significant']] == T, 'B_zscore']))
# and the max z
...
max_sig_z_mo <- max(abs(b_combined_top_and_lcl_caqtl_mosig[b_combined_top_and_lcl_caqtl_mosig[['B_significant']] == T, 'B_zscore']))

# calculate concordance
n_significant_both <- sum(b_combined_top_and_lcl_caqtl_mosig[['lcl_significant']] == T & b_combined_top_and_lcl_caqtl_mosig[['B_significant']] == T)
n_significant_directed_both <- sum(b_combined_top_and_lcl_caqtl_mosig[['lcl_significant']] == T & b_combined_top_and_lcl_caqtl_mosig[['B_significant']] == T &
                                     sign(b_combined_top_and_lcl_caqtl_mosig[['lcl_zscore_signed']]) == sign(b_combined_top_and_lcl_caqtl_mosig[['B_zscore']]))
concordance <- round(n_significant_directed_both / n_significant_both, digits = 3)

# plot the concordance of the two
p <- ggplot(data = b_combined_top_and_lcl_caqtl_mosig, mapping = aes(x = B_zscore, y = lcl_zscore_signed)) + 
  geom_point(size = 0.2) + 
  ...
  xlab('Z-score in multi-omics B caQTLs') +
  ...
  annotate("text", x = max_sig_z_mo * -0.70 , y = max_sig_z_lcl * 0.75, label = 'discordant', colour = '#D55E00', fontface = 'bold', size = 18/.pt) +
  # add the names of the concordant and non-concordant blocks
  annotate("text", x = max_sig_z_mo * 0.70 , y = max_sig_z_lcl * 0.75, label = 'concordant', colour = '#0072B2', fontface = 'bold',  size = 18/.pt) +
  # add the title
  ggtitle('Concordance of caQTLs between\nLCL and multi-omics B cells') + 
  # increase legend sizes
  theme(
    # X label font size
    axis.title.x = element_text(size = 24), 
    # Y label font size
...
# save this plot
ggsave('~/plots/mo_caqtl_scatter_mo_b_combined_vs_lcl.pdf', width=8, height=8)
