...
eqtl_caqtl_coloc_signals_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/caqtl/sc-eqtlgen/output_cis_50kb_updated_features/coloc_ieqtl_iecaql/subset_to_uodated_FDR_annotatin.tsv'
# location of variants
variant_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/genotype/imputed_hg38_all_anc_qtl_tested_variants'
# location of the eQTL inputs
eqtl_input_split_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/input/L1/'
eqtl_input_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/interaction_eqtl/sc-eqtlgen/input/L1/'
# location of the caQTL inputs
caqtl_input_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/caqtl/sc-eqtlgen/input/L1/'
# and the sample mapping files
eqtl_smf_loc <- paste('/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/interaction_eqtl/sc-eqtlgen/input/L1/combined/', '/smf.txt', sep = '/')
caqtl_smf_loc <- paste('/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/interaction_caqtl/sc-eqtlgen/input/L1/', '/smf.txt', sep = '/')
...
eqtl_files_per_cond <- list(
  'UT' = read_expression_files_per_celltype(paste(eqtl_input_split_loc, 'UT/', sep = '/')), 
  '24hCA' = read_expression_files_per_celltype(paste(eqtl_input_split_loc, '24hCA/', sep = '/')), 
  'combined' = read_expression_files_per_celltype(paste(eqtl_input_loc, 'combined/', sep = '/'))
)
...
```{r geneplot, include=TRUE, fig.height=5, fig.width=10}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

gene <- 'ERAP2'
variant <- '5:96893521:G:T'
cell_type <- 'monocyte'

plot_grid(
  plot_qtls(gene, variant, feature_table = eqtl_files_per_cond[['combined']][[cell_type]][, intersect(colnames(eqtl_files_per_cond[['combined']][[cell_type]]), colnames(eqtl_files_per_cond[['UT']][[cell_type]]))], genotypes = genotypes, sample_mapping = eqtl_smf, better_colors = T, legendless = T) + ggtitle(paste(variant, 'affecting', gene, 'expression\nin', cell_type)) + xlab('genotype') + ylab(paste(gene, 'expression')) + ylim(c(-4, 4)), 
  plot_qtls(gene, variant, feature_table = eqtl_files_per_cond[['combined']][[cell_type]][, intersect(colnames(eqtl_files_per_cond[['combined']][[cell_type]]), colnames(eqtl_files_per_cond[['24hCA']][[cell_type]]))], genotypes = genotypes, sample_mapping = eqtl_smf, better_colors = T, legendless = T) + ggtitle(paste(variant, 'affecting', gene, 'expression\nin', cell_type)) + xlab('genotype') + ylab(paste(gene, 'expression')) + ylim(c(-4, 4))
)

```


```{r coloc_plotss, include=TRUE, fig.height=5, fig.width=10}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# plot the eQTL and caQTL that are opposite direction
p_eqtl_opposite <- plot_qtls('ARL17B', '17:45614972:T:C', feature_table = eqtl_files_per_cond$UT$monocyte, genotypes = genotypes, sample_mapping = eqtl_smf, better_colors = T, legendless = T) + ggtitle('rs2668709 affecting ALR17B expression\nin monocytes') + xlab('genotype') + ylab('ALR17B expression')
p_caqtl_opposite <- plot_qtls('chr17-45585724-45587049', '17:45614972:T:C', feature_table = caqtl_files_per_cond$UT$monocyte, genotypes = genotypes, sample_mapping = eqtl_smf, better_colors = T, legendless = T) + ggtitle('rs2668709 affecting chr17-45585724-45587049\naccessibility in monocytes') + xlab('genotype') + ylab('chr17-45585724-45587049 accessibility')
...
p_eqtl_same <- plot_qtls('AL139042.1', '6:88449573:C:G', feature_table = eqtl_files_per_cond$UT$monocyte, genotypes = genotypes, sample_mapping = eqtl_smf, better_colors = T, legendless = T) + ggtitle('rs9353559 affecting AL139042.1 expression\nin monocytes') + xlab('genotype') + ylab('AL139042.1 expression')
p_caqtl_same <- plot_qtls('chr6-88442655-88443459', '6:88449573:C:G', feature_table = caqtl_files_per_cond$UT$monocyte, genotypes = genotypes, sample_mapping = eqtl_smf, better_colors = T, legendless = T) + ggtitle('rs9353559 affecting chr6-88442655-88443459\naccessibility in monocytes') + xlab('genotype') + ylab('chr6-88442655-88443459 accessibility')
...
# save them also in PDF format
dir.create('~/plots/', showWarnings = F, recursive = T)
ggsave('~/plots/mo_p_eqtl_opposite.pdf', p_eqtl_opposite, width = 5, height = 5)
ggsave('~/plots/mo_p_caqtl_opposite.pdf', p_caqtl_opposite, width = 5, height = 5)
ggsave('~/plots/mo_p_eqtl_same.pdf', p_eqtl_same, width = 5, height = 5)
ggsave('~/plots/mo_p_caqtl_same.pdf', p_caqtl_same, width = 5, height = 5)

```

```{r interaction_plots, include=TRUE, fig.height=5, fig.width=10}

# plot the opposite effect as a scatterplot
p_scatter_qtl_opposite <- plot_interactions('chr17-45585724-45587049', 'ARL17B', '17:45614972:T:C', caqtl_files_per_cond$UT$monocyte, eqtl_files_per_cond$UT$monocyte, genotypes, caqtl_smf, eqtl_smf, better_colors = T) + ggtitle('chr17-45585724-45587049 accessibility\nvs ALR17B expression in monocytes\nper rs2668709 genotype') + xlab('chr17-45585724-45587049 accessibility') + ylab('ALR17B expression') +
  guides(color = guide_legend('genotype'))
# plot the same direction effect as a scatterplot
p_scatter_qtl_same <- plot_interactions('chr6-88442655-88443459', 'AL139042.1', '6:88449573:C:G', caqtl_files_per_cond$UT$monocyte, eqtl_files_per_cond$UT$monocyte, genotypes, caqtl_smf, eqtl_smf, better_colors = T) + ggtitle('chr6-88442655-88443459 accessibility\nvs AL139042.1 expression in monocytes\nper rs9353559 genotype') + xlab('chr6-88442655-88443459 accessibility') + ylab('AL139042.1 expression') +
  guides(color = guide_legend('genotype'))

# plot them together
plot_grid(
  p_scatter_qtl_opposite, 
  p_scatter_qtl_same
)

# save them also in PDF format
dir.create('~/plots/', showWarnings = F, recursive = T)
ggsave('~/plots/mo_p_scatter_qtl_opposite.pdf', p_scatter_qtl_opposite, width = 5, height = 5)
ggsave('~/plots/mo_p_scatter_qtl_same.pdf', p_scatter_qtl_same, width = 5, height = 5)
