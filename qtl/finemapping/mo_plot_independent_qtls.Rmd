...
```{r count_additional_cs, include=TRUE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# add ct to each result
for (ct in names(eqtl_outputs_filtered)) {
  eqtl_outputs_filtered[[ct]][['cell_type']] <- ct
}
# get the unique combinations of credible set, genes and cell type
eqtl_cs_gene_ct <- unique(do.call('rbind', eqtl_outputs_filtered)[, c('feature_id', 'CS', 'cell_type')])
# just set the CS to be L1 when missing
eqtl_cs_gene_ct[is.na(eqtl_cs_gene_ct[['CS']]), ][['CS']] <- 'L1'
# now get the unique combinations of the CS to gene
eqtl_cs_gene <- unique(eqtl_cs_gene_ct[, c('CS', 'feature_id')])
# get the total of genes
n_gene <- length(unique(eqtl_cs_gene[['feature_id']]))
# get the total number of effects
n_gene_cs <- nrow(eqtl_cs_gene)
# the difference is the number of additional effects
n_gene_additional <- n_gene_cs - n_gene
# show the number of effects
n_gene
# [1] 1571
...
qtl_coloc_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/colocalization/eqtl_caqtl/ut_and_24hca_significant/'
# read this data
...
overlapping_effects_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/eqtl_caqtl_overlap/combined/L1/'
# check each cell type
...
combined_overlapping_region_to_gene_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/cre_eqtl/eqtl_caqtl_overlap/combined/confinements/region_to_peak_variant_overlaps.tsv.gz'
# and write
...
```{r n_cts_qtls, include=TRUE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# get the unique egenes for each celltype
egenes_ct_l <- list()
# by checeking each cell type
for (ct in names(eqtl_outputs)) {
  # get the unique features
  ct_features <- unique(eqtl_outputs[[ct]][['feature_id']])
  # make into df
  ct_features_df <- data.frame('cell_type' = rep(ct, times = length(ct_features)), 'feature_id' = ct_features)
  # put in the list
  egenes_ct_l[[ct]] <- ct_features_df
}
# merge all of them
egenes_ct <- do.call('rbind', egenes_ct_l)
# get how many cell types these are present
egenes_ct_occ <- data.frame(table(egenes_ct[['feature_id']]))
# set column names
colnames(egenes_ct_occ) <- c('feature_id', 'n_celltypes')

# get the unique capeaks for each celltype
capeaks_ct_l <- list()
# by checeking each cell type
for (ct in names(eqtl_outputs)) {
  # get the unique features
  ...
  ct_features_df <- data.frame('cell_type' = rep(ct, times = length(ct_features)), 'feature_id' = ct_features)
  # put in the list
  capeaks_ct_l[[ct]] <- ct_features_df
}
# merge all of them
capeaks_ct <- do.call('rbind', capeaks_ct_l)
# get how many cell types these are present
capeaks_ct_occ <- data.frame(table(capeaks_ct[['feature_id']]))
# set column names
colnames(capeaks_ct_occ) <- c('feature_id', 'n_celltypes')
