...
############################################################################################################################
# Authors: Roy Oelen
# Name: mo_plot_ieqtl_examples.R
# Function: plot top i-eQTLs that are also DE
# Example: 
# Rscript 
############################################################################################################################
...
# the confinement
confinement_loc <- '~/tables/mo_ieqtl_plot_tbl.tsv.gz'
...
# location of all cell metadata
cell_mtdt_loc <- '/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/metadata/mo_celllevel_metadata.tsv.gz'
# read that
cell_mtdt <- fread(cell_mtdt_loc, header = T, sep = '\t')
# make a mapping of lane to sample to condition
lane_to_sample_condition <- unique(cell_mtdt[, c('lane', 'sample_final', 'condition_final')])
# add also the naming as present for i-eQTL inputs
lane_to_sample_condition[['sample_lane']] <- paste(lane_to_sample_condition[['sample_final']], lane_to_sample_condition[['lane']], sep = ';;')
...
for (i in 1 : nrow(confinement)) {
  ...
  gene_min <- min(eqtl_inputs[[cell_type]][gene, ], na.rm = T)
  # and max value
  gene_max <- max(eqtl_inputs[[cell_type]][gene, ], na.rm = T)
  # get the feature table
  feature_tbl_ct <- eqtl_inputs[[cell_type]]
  # get the UT
  feature_tbl_ct_ut <- feature_tbl_ct[
    , intersect(
        colnames(feature_tbl_ct), 
        lane_to_sample_condition[!is.na(lane_to_sample_condition[['condition_final']]) & lane_to_sample_condition[['condition_final']] == 'UT', ][['sample_lane']]
      )
  ]
  # make the gene plot
  p_gene_ut <- plot_qtls(gene, 
                      variant, 
                      feature_table = feature_tbl_ct_ut, 
                      genotypes = genotypes, 
                      sample_mapping = eqtl_smf, 
                      better_colors = T, legendless = T) + 
    ggtitle(paste(variant, 'affecting\n', gene, 'expression in', cell_type, 'UT')) + xlab(paste(variant, 'genotype')) + ylab(paste(gene, 'expression')) +
    ylim(c(gene_min, gene_max))
  # get the 24hCA
  feature_tbl_ct_24hca <- feature_tbl_ct[
    , intersect(
      colnames(feature_tbl_ct), 
      lane_to_sample_condition[!is.na(lane_to_sample_condition[['condition_final']]) & lane_to_sample_condition[['condition_final']] == '24hCA', ][['sample_lane']]
    )
  ]
  # get 24hCA
  p_gene_24hCA <- plot_qtls(gene, 
                         variant, 
                         feature_table = feature_tbl_ct_24hca, 
                         genotypes = genotypes, 
                         sample_mapping = eqtl_smf, 
                         better_colors = T, legendless = T) + 
    ggtitle(paste(variant, 'affecting\n', gene, 'expression in', cell_type, '24hCA')) + xlab(paste(variant, 'genotype')) + ylab(paste(gene, 'expression')) +
    ylim(c(gene_min, gene_max))
  
  # and both
  p_both <- plot_grid(
    p_gene_ut, p_gene_24hCA,
    nrow = 1,
    ncol = 2
  )
  # put in the list
  ieqtl_plots[[paste(variant, gene, cell_type, sep = '_')]] <- p_both
}

# let's try to save the plots
ieqtl_plots_loc <- '~/multiome/plots/ieqtls/'
# make that directory
dir.create(ieqtl_plots_loc, recursive = T)
# now do each combination
for (comb in names(ieqtl_plots)) {
  # extract the plot
  p_to_plot <- ieqtl_plots[[comb]]
  # make a safer name
  p_name <- gsub('\\:', '.', comb)
  # save the plot
  ggsave(paste0(ieqtl_plots_loc, '/', p_name, '.pdf'), plot = p_to_plot, width = 6, height = 6)
}
