...
############################################################################################################################
# Authors: Roy Oelen
# Name: mo_eigenmt_correct_limix_qtls_nc2022.R
...
# genotypes 
genotypes_loc <- '/groups/umcg-franke-scrna/tmp02/projects/sc-eqtlgen-consortium-pipeline/wg1-preprocessing/wg1_wijst2020/genotype/imputed/plink_converted/'

# location of the eQTL interactions
eqtl_interaction_loc <- '/groups/umcg-franke-scrna/tmp02/projects/multiome/ongoing/qtl/interaction_eqtl/sc-eqtlgen/replication/output/nc2022/ut_and_24hca_significant/24hCA/'

# check each cell type in the eQTLs
eqtl_interactions_per_celltype <- list()
# list the cell types
cell_types <- list.dirs(eqtl_interaction_loc, recursive = F, full.names = F)
# check each cell type
for (ct in c('DC', 'monocyte', 'NK')) {
  print(ct)
  ct_res <- correct_all_chunks_all_chromosomes(
    genotype_loc = genotypes_loc, 
    chunks_loc = paste(eqtl_interaction_loc, '/', ct, '/condition_coded/qtl/', sep = ''), 
    chromosomes = 1:22, 
    genotype_prepend = 'imputed_hg38_info_filled_chr', 
    genotype_append = '', 
  ...
  out_loc = paste(eqtl_interaction_loc, '/', ct, '/condition_coded/iqtl_results_all_eigenmt.tsv.gz', sep = '')
  # zip it
