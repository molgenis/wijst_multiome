...
###################################################################
#Script Name	  : mo_create_limma_dar_jobs.sh
...
#Author       	: Roy Oelen
#example        : ./mo_create_limma_dar_jobs.sh \
...
script_loc='/groups/umcg-franke-scrna/tmp04/users/umcg-roelen/singularity/rstudio-server/simulated_home/mo_differential_accessibility_limma_parameterised.R'
...
for object in ${REGEX_SEURAT_OBJECTS}
  ...
  for i in "${iterations[@]}"
    do
        ...
        if [ ${PERMUTE} -eq 1 ]
            then
                JOB_NAME='limma_'${file_basename}'_permutation_'${i}
            fi
