...
############################################################################################################################
# Authors: Roy Oelen
# Name: mo_differential_accessibility_kimma_parameterised.R
# Function: perform differential gene expression analysis using Kimma
############################################################################################################################
...
dream_pairwise_mt <- function(seurat_object, kinship, output_loc, condition_combinations, aggregates=c('sample_final', 'condition_final'), fixed_effects=c('condition_final'), random_effects=c('sample_final'), kin_column='sample_final', minimal_cells=0, min_peaks=200, minimal_complexity=5000, min_pct=0.01, verbose=T, plot_metrics=T, nthreads=5){
  # set assay
  ...
  # filter duplicated entries. Should not be possible, but let's check anyway
  n_per_rowname <- data.frame(table(rownames(aggregate_countMatrix)))
  rownames_bigger_1 <- n_per_rowname[n_per_rowname[['Freq']] > 1, 'Var1']
  # remove entries that have more than one
  aggregate_countMatrix <- aggregate_countMatrix[!(rownames(aggregate_countMatrix) %in% rownames_bigger_1), ]
  # rename
  gsub('-', '.', rownames(aggregate_countMatrix))
...
#' @param fixed_effects vector of fixed effects to include in the model
#' @param random_effects vector or random effects to include in the model
#' @param kin_column column in metadata denoting the sample name matching the kinship columns and rows
#' @param minimal_cells the minimal number of cells that needs to be present for a sample to not be excluded
#' @param min_peaks the minimal number of UMIs a cell must have to be used for the pseudobulk
#' @param minimal_complexity the minimal number of cells transcripts a pseudobulk needs to be based on for a sample to not be excluded
#' @param verbose whether to print progress messages
...
do_debug <- function() {
  # fill the opt
  opt <- list()
  opt[['out']] <- '/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/differential_accessibility/kimma/output/stimulation/pct01/nolane/'
  opt[['file']] <- '/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/cpeaks_peak_calling/signac/rounded/mo_cpeaks_filtered_b_wstatus_1_80_20240701.rds'
  opt[['kin']] <- '/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/genotype/kinship/unique.sample.kinship'
