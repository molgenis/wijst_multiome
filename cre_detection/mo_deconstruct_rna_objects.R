...
deconstruct_seurat_object <- function(seurat_object_loc, output_folder='./') {
    ...
    cell_barcodes <- colnames(metadata)

    # create the root folder
    dir.create(output_folder, recursive = T)

    # write those barcodes
    write.table(data.frame(x = cell_barcodes), gzfile(paste(output_folder, '/barcodes.tsv.gz', sep = '')), row.names = F, col.names = F, quote = F)
    ...
    features_present_sct <- rownames(seurat_object@assays$SCT@meta.features)
...
write_deconstructed_seurat_object <- function(input_dir, output_dir='./', seurat_objects_prepend='^mo_all_20240619_seuratv5_annotated_agesexcovid_', seurat_objects_append='\\.rds$') {
