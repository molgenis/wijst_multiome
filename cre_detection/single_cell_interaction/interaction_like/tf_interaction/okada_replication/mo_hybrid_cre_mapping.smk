...
# set rule for container
RULE_CONTAINER = CONTAINER if CONTAINER else None
...
def resolve_rel_or_abs(base_dir, maybe_rel_path):
    ...
    elif maybe_rel_path.startswith("./"):
        return os.path.abspath(maybe_rel_path)
...
rule all:
    input:
        # each chunk result
        expand(os.path.join(RESULTS_BASE, "{chunk}", "result.tsv.gz"), chunk = CHUNKS),
        # merge results
        os.path.join(RESULTS_BASE, "merged/results_fdr.tsv.gz")
...
rule _run_interaction:
    ...
    output:
        tsv = os.path.join(RESULTS_BASE, "{chunk}", "result.tsv.gz")
    params:
...
if CONTAINER:
    use rule _run_interaction as run_interaction with:
        container:
            CONTAINER
    ...
    use rule _run_interaction as run_interaction
...
rule _merge_results:
    input:
        ...
        expand(os.path.join(RESULTS_BASE, "{chunk}", "result.tsv.gz"), chunk = CHUNKS)
    output:
        merged = os.path.join(RESULTS_BASE, "merged/results_fdr.tsv.gz")
    params:
...
if CONTAINER:
    use rule _merge_results as merge_results with:
        container:
            CONTAINER
    ...
    use rule _merge_results as merge_results
