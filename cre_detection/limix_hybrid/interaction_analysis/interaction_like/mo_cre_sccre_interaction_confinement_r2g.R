...
############################################################################################################################
# Authors: Roy Oelen
# Name: mo_cre_sccre_interaction_confinement_r2g.R
# Function: create variant-region-gene confinements for chromatin region interaction-eQTL mapping
# 
############################################################################################################################
...
# add the region to the top QTLs
eqtl_top_var_feature_per_cs_r <- merge(eqtl_top_var_feature_per_cs, scenic_output[, c('Gene', 'region_cpeaks')], by.x = 'feature', by.y = 'Gene')
# keep across all cell types
eqtl_top_var_feature_per_cs_r_all <- unique(eqtl_top_var_feature_per_cs_r[, c('variant', 'region_cpeaks', 'feature')])
# set column names
colnames(eqtl_top_var_feature_per_cs_r_all) <- c('variant', 'region', 'feature')

# add the gene to the top QTLs
caqtl_top_var_feature_per_cs_r <- merge(caqtl_top_var_feature_per_cs, scenic_output[, c('Gene', 'region_cpeaks')], by.x = 'feature', by.y = 'region_cpeaks')
# keep across all cell types
caqtl_top_var_feature_per_cs_r_all <- unique(caqtl_top_var_feature_per_cs_r[, c('variant', 'feature', 'Gene')])
# set column names
colnames(caqtl_top_var_feature_per_cs_r_all) <- c('variant', 'region', 'feature')

# merge the gene-TF results from the eQTLs and caQTLs
qtl_top_var_feature_per_cs_r_all_both <- unique(rbind(
  eqtl_top_var_feature_per_cs_r_all, 
