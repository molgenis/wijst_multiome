...
```{r gene_exp_chrom_openness, include=TRUE, fig.width=8, fig.height=8}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# add the average openness
cd8t_openness_loc <- '/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/signac_peaks/output/mo_peaks_lane1to80_CD8T.bed'
cd8t_openness <- fread(cd8t_openness_loc, header = T, sep = '\t')
# get non-zero expression
cd8t_nonzero_loc <- '/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/ncells_by_genes/L1/combined/CD8T.tsv.gz'
cd8t_nonzero <- fread(cd8t_nonzero_loc, header = T, sep = '\t')
# make this per feature
cd8t_nonzero <- data.frame('feature' = cd8t_nonzero[['feature']], 'pct_exp' = rowSums(cd8t_nonzero[, -c('feature')]) / cd8t_openness[1, ][['ncell']])

```

```{r sampling_filtering, include=TRUE, fig.width=8, fig.height=8}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# we'll filter on the expression side
expression_frac_cutoff <- 0.1

# create a new list to save the filtered lists
subsampling_replication_cd8t_filtered <- list()
# check each of the sets
for(comparison in names(subsampling_replication_cd8t)) {
  # extract the comparison that we are looking at
  subsampling_specific <- subsampling_replication_cd8t[[comparison]]
  # add the pct
  subsampling_specific[['frac_exp']] <- cd8t_nonzero[match(gsub('_\\S+$', '', subsampling_specific[['qtl']]), cd8t_nonzero[['feature']]), ][['pct_exp']]
  # subset the data on the cutoff
  subsampling_specific <- subsampling_specific[
    subsampling_specific[['frac_exp']] >= expression_frac_cutoff, 
  ]
  # put this into the list
  subsampling_replication_cd8t_filtered[[comparison]] <- subsampling_specific
}
