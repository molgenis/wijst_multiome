...
# and without major jitter
p_auc_ct_major_nojitter <- ggplot(data = NULL, mapping = aes(x = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][['cell_type_nice_ordered']], y = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][[ereg]], fill= auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][['cell_type_nice_ordered']])) +
  geom_boxplot(outlier.shape = NA) + 
  # and add jitter
  # geom_jitter(size = 0.5, alpha = 0.1) +
  # and labels
...
p_auc_ct_major <- p_auc_ct_major  +
      theme(legend.title = element_text(size=14), 
            ...
            axis.text.x = element_text(size=14),
            strip.text.x = element_text(size=14))
p_auc_ct_major_nojitter  <- p_auc_ct_major_nojitter  +
      theme(legend.title = element_text(size=14), 
            legend.text = element_text(size=14),
            axis.title.x = element_text(size=16),
            axis.title.y = element_text(size=16),
...
p_auc_ct
p_auc_ct_major
p_auc_ct_major_nojitter

# and save them
ggsave(filename = '~/multiome/plots/mo_tf_activity_ebf1_ctmajor.pdf', plot = p_auc_ct_major, width = 10, height = 5)
ggsave(filename = '~/multiome/plots/mo_tf_activity_ebf1_ctmajor_nojitter.pdf', plot = p_auc_ct_major_nojitter, width = 6, height = 6)
...
```{r plot_activity_b_cond, include=TRUE, fig.width=5, fig.height=5}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# plot eregs for major cell types
eregs_vector <- c("KLF10_direct_+/+_(59g)", "KLF12_direct_+/+_(1093g)", "KLF12_direct_-/+_(464g)", "KLF3_direct_+/+_(256g)", "KLF3_direct_-/+_(18g)", "KLF6_direct_+/+_(419g)", "KLF6_direct_-/+_(27g)", "KLF9_direct_+/+_(14g)", "PLAG1_direct_+/+_(25g)", "REL_direct_-/+_(35g)")
for (ereg in eregs_vector) {
  p_specific_ereg <- ggplot(data = NULL, mapping = aes(x = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][['cell_type_nice_ordered']], y = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][[ereg]], fill= auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][['cell_type_nice_ordered']])) +
  geom_boxplot(outlier.shape = NA) + 
  # and add jitter
  geom_jitter(size = 0.5, alpha = 0.1) +
  # and labels
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  scale_fill_manual(values = get_color_coding_dict())
  print(p_specific_ereg)
}

```

```{r plot_activity_repl_cond, include=TRUE, fig.width=5, fig.height=5}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)

# plot eregs for major cell types
eregs_vector <- c("KLF10_direct_+/+_(59g)", "KLF12_direct_+/+_(1093g)", "KLF12_direct_-/+_(464g)", "KLF3_direct_+/+_(256g)", "KLF3_direct_-/+_(18g)", "KLF6_direct_+/+_(419g)", "KLF6_direct_-/+_(27g)", "KLF9_direct_+/+_(14g)", "PLAG1_direct_+/+_(25g)", "REL_direct_-/+_(35g)")
for (ereg in eregs_vector) {
  p_specific_ereg <- ggplot(data = NULL, mapping = aes(x = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][['celltype_condition_ordered']], y = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][[ereg]], fill = auc_dt[auc_dt[['cell_type_nice_ordered']] %in% major_cts, ][['celltype_condition_ordered']])) +
  geom_boxplot(outlier.shape = NA) + 
  # and add jitter
  geom_jitter(size = 0.5, alpha = 0.1) +
  # and labels
  ...
  print(p_specific_ereg)
  # save plot
  ggsave(file = paste0('~/multiome/plots/tf_activity/', gsub('/', '_', ereg), '.pdf'), plot = p_specific_ereg, width = 5, height = 5)
}
...
for (ereg in eregs_vector) {
  ...
  print(p_specific_ereg)
  # save plot
  ggsave(file = paste0('~/multiome/plots/tf_activity/', gsub('/', '_', ereg), '_nodot.pdf'), plot = p_specific_ereg, width = 5, height = 5)
}
