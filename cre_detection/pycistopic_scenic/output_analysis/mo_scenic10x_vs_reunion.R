...
############################################################################################################################
# Authors: Roy Oelen
# Name: mo_scenic10x_vs_reunion.R
# Function: compare SCENIC+ 10x output to REUNION output
############################################################################################################################
...
scenic_output_loc <- '/groups/umcg-franke-scrna/tmp04/external_datasets/10x_multiome_10k_pbmcs/scenic/outs/eRegulons_both.tsv.gz'
...
ggvenn::ggvenn(
  data = list('10x10k' = unique(scenic_output[['Region']]), 'REUNION' = unique(reunion_output[['overlapping_feature']]))
) + ggtitle('Overlap of CREs between 10x10k and REUNION')
ggvenn::ggvenn(
  data = list('10x10k' = unique(scenic_output[['Gene']]), 'REUNION' = unique(reunion_output[['gene_id']]))
) + ggtitle('Overlap of CRE genes between 10x10k and REUNION')
ggvenn::ggvenn(
  data = list('10x10k' = unique(paste(scenic_output[['Region']], scenic_output[['Gene']])), 'REUNION' = unique(paste(reunion_output[['overlapping_feature']], reunion_output[['gene_id']])))
) + ggtitle('Overlap of region-gene links between 10x10k and REUNION')
ggvenn::ggvenn(
  data = list('10x10k' = unique(paste(scenic_output[['Region']], scenic_output[['Gene']], scenic_output[['TF']])), 'REUNION' = unique(paste(reunion_output[['overlapping_feature']], reunion_output[['gene_id']], reunion_output[['motif_id']])))
) + ggtitle('Overlap of triplets between 10x10k and REUNION')
...
ggplot(data = matched_output, mapping = aes(x = cor_scenic, y = cor_reunion)) + 
  geom_point() +
  xlab('R2G Rho in 10x10k') + 
  ylab('R2G Rho in reunion') + 
  ggtitle('R2G correlations in 10x10k vs reunion (matched TF only)') + 
