...
# Name: mo_create_limix_qtl_input_l2.R
...
inverse_normalize <- function(norm_count_matrix, verbose = T) {
  ...
  # we also need that last row, so we have to round up
...
#' @param single_thread do not multi-thread, but do all the work on one CPU (easier for debugging purposes)
...
do_limix_input_pipeline(
  ...
  output_loc='/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/input/L1/UT/',
...
do_limix_input_pipeline(
  ...
  psam = donor_annotation_psam, 
  output_loc='/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/input/L1/24hCA/',
  participant_column='sample_final',
  celltype_column='celltype_imputed_lowerres',
  merge_pcs_into_covariates=F,
  join_pools=F,
...
do_limix_input_pipeline(
  seurat_object = seurat_object, 
  psam = donor_annotation_psam, 
  output_loc='/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/input/L1/combined/',
  participant_column='sample_final',
  celltype_column='celltype_imputed_lowerres',
  merge_pcs_into_covariates=F,
  join_pools=F,
...
# location of annotation for cell types at higher resolution
...
seurat_object <- AddMetaData(seurat_object, azi_pbmc_annotations[, colnames(azi_pbmc_annotations)])
# make the l2 cell types without illegal characters
seurat_object@meta.data[['predicted.azipbmc.l2.safe']] <- make_celltypes_safe(seurat_object@meta.data[['predicted.azipbmc.l2']])

# subset to the unstimulated samples
do_limix_input_pipeline(
  seurat_object = seurat_object[, !is.na(seurat_object@meta.data[['inflammation_final']]) & seurat_object@meta.data[['inflammation_final']] == 'UT'], 
  psam = donor_annotation_psam, 
  output_loc='/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/input/L2/UT/',
  participant_column='sample_final',
  celltype_column='predicted.azipbmc.l2.safe',
  merge_pcs_into_covariates=F,
  join_pools=F,
  min_numi=200,
  min_sample_cor=0.7,
...
do_limix_input_pipeline(
  seurat_object = seurat_object[, !is.na(seurat_object@meta.data[['inflammation_final']]) & seurat_object@meta.data[['inflammation_final']] == '24hCA'], 
  psam = donor_annotation_psam, 
  output_loc='/groups/umcg-franke-scrna/tmp04/projects/multiome/ongoing/qtl/eqtl/sc-eqtlgen/input/L2/24hCA/',
  participant_column='sample_final',
  celltype_column='predicted.azipbmc.l2.safe',
  merge_pcs_into_covariates=F,
  join_pools=F,
